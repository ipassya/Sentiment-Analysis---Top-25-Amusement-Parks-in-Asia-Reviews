...
clean_data <- function(data) {
  ...
  # VCorpus sebuah objek R. daftar parameter kontrol bernama untuk membaca konten dari x. sebuah fungsi yang mampu membaca dan memproses format yang dikirimkan oleh x. karakter yang memberi bahasa (sebaiknya sebagai tag bahasa IETF, lihat bahasa dalam paket NLP).
  # fungsi tm_map () digunakan untuk menghapus spasi yang tidak perlu, untuk mengubah teks menjadi huruf kecil, untuk menghapus kata kunci umum seperti 'the', menghapus kata-kata semacam ini berguna sebelum analisis lebih lanjut.
...
# Fungsi dtm mengembalikan vektor panjang (jumlah titik yang disimpan dalam Grid) yang berisi nilai jarak untuk mengukur fungsi yang dievaluasi pada setiap titik Grid.
...
train_model <- function() {
  ...
  file_path <- "tripadvisor-amusement-parks-traning-dataset.txt"
  ...
  # Set. seed () fungsi mengatur nomor awal yang digunakan untuk menghasilkan urutan nomor acak - ini memastikan bahwa Anda mendapatkan hasil yang sama jika Anda memulai dengan seed yang sama setiap kali Anda menjalankan proses yang sama.
  set.seed(1)
  ...
  # Fungsi nrow () digunakan untuk mengembalikan jumlah baris dari matriks yang ditentukan.
  # round membulatkan nilai dalam argumen pertamanya ke jumlah tempat desimal yang ditentukan (default 0).
